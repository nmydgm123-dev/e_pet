class Pet {
  ...
  renderCat(x, y, breathe) {
    ...
    // Body (front view)
    const bodyGrad = this.ctx.createRadialGradient(x, y, 0, x, y, finalSize * 0.9);
    bodyGrad.addColorStop(0, '#FFFFFF');
    bodyGrad.addColorStop(0.6, '#FFF8F8');
    bodyGrad.addColorStop(1, '#FFE4EC');
    ...
    // Head (big round head)
    const headGrad = this.ctx.createRadialGradient(x - finalSize * 0.2, y - finalSize * 0.4, 0, x, y - finalSize * 0.3, finalSize * 0.7);
    headGrad.addColorStop(0, '#FFFFFF');
    headGrad.addColorStop(1, '#FFF0F5');
    this.ctx.fillStyle = headGrad;
    this.ctx.beginPath();
    this.ctx.arc(x, y - finalSize * 0.35, finalSize * 0.65, 0, Math.PI * 2);
    this.ctx.fill();
    
    // Ears (triangular cute ears)
    ...
    // Paws (front view - just small ovals at bottom)
    if (this.state === 'stretching') {
      const stretchOffset = Math.sin(this.animationFrame * 0.2) * finalSize * 0.3;
      this.ctx.fillStyle = '#FFF';
      this.ctx.beginPath();
      this.ctx.ellipse(x - finalSize * 0.5, y + finalSize * 0.7 + stretchOffset, finalSize * 0.2, finalSize * 0.12, 0, 0, Math.PI * 2);
      this.ctx.fill();
      this.ctx.beginPath();
      this.ctx.ellipse(x + finalSize * 0.5, y + finalSize * 0.7 + stretchOffset, finalSize * 0.2, finalSize * 0.12, 0, 0, Math.PI * 2);
      this.ctx.fill();
